# Handoff: tek origin yayında, v0.20.0 OTA'da, katalog bilgisi kalıcı

> 2026-09-20 15:31 · `dev` @ `ef26e9f` (origin/dev ile aynı) · çalışma ağacı temiz
...
- `3850d47` merge
- `4636b65` chore(release): v0.20.0 (`variables.gradle` 0.19.0 → 0.20.0)
- `499dddd` feat(ops): `publish_ota.mjs` — OTA katalog yayını tek komut
- `04f16bb` / `54c0477` / `ef26e9f` docs(handoff)

**Repo dışına yazılanlar** (bir daha aranmasın diye, Dean'in isteğiyle):
- `~/.ai/guides/evaitec-ota-catalog.md` — kataloğun adresi, `apps.json` ≠ `latest.json`,
  şema alanlarının ne bozduğu, yeni APK koyma reçetesi, Pages önbellek tuzağı.
  `INDEX.md` satır 51'de kayıtlı; `android-client-engine-ota` ve `wear-app-bootstrap-ota`
  rehberlerinin başına çapraz bağ düştü. `~/.ai` commit `618bcc8`.
- Proje hafızası: `ota-ekle-komutu` (Dean **"ekle"** dediğinde = yeni APK'leri
  evaitecOTA'ya yayınla, sorma) ve `evaitecota-katalog-yeri`. İkisi de `MEMORY.md`'de.
- Skill indeksi yeniden derlendi (235 kayıt); `skilldb search "evaitecOTA katalog apk ekle"`
  rehberi birinci sırada getiriyor.
...
```bash
git rev-parse --short HEAD          # ef26e9f
...
```
life-os-wellness (D:\projects\evaitec\lifeOS\life-os-wellness), dal dev @ ef26e9f, ağaç temiz.
...
v0.20.0 APK'si evaitecOTA katalogunda (evaglass-releases/apps.json); katalog ayri
depoda ve `node ops/publish_ota.mjs` ile guncelleniyor, tag atmak yetmiyor.
Dean "ekle" derse kastettigi budur (proje hafizasinda kayitli).
