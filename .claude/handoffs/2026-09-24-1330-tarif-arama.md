...
## Ek (13:30) — elimdekilerle web aramalı tarif (`1a8f1d7`, `v0.33.0`)
- Kök neden (doğrulandı, canlı): LiteLLM Gemini arama kaynaklarını `vertex_ai_grounding_metadata[].groundingChunks[].web`
  içinde döndürüyor; `collectSources` yalnız `annotations` okuyordu → kaynak hep 0. `groundingSources()` eklendi.
- `wantsRecipes()` (tarif/elimde/malzeme/ne yap): fotoğraflı mesajda da arama + `RECIPE_HINT`; kaynaksız cevapta 1 yeniden deneme;
  `preview.ts` kaynakların og:image'ını çeker (yalnız vertexaisearch yönlendirmesi takip edilir — SSRF koruması).
- Canlı ölçüm: 2 deneme → 2 kaynak/2 resim (24.9 sn), 1 kaynak/1 resim (10.1 sn). API testleri 68+6 pass, web 372 pass, typecheck 0.
- Eva kaynakları küçük resimle (APK 0.33.0 gerek); `tools/secici/tatli.html` "Elimdekiler" sekmesi (yazı+foto, token localStorage).
- Doğrulanmadı: tatli.html Elimdekiler sekmesi tarayıcıda uçtan uca denenmedi; 0.33.0 OTA yayını arka planda (bt0nwb33v).
- Tuzak: `docker compose exec` Git Bash'te yol çevirir → `MSYS_NO_PATHCONV=1`. API rebuild sonrası /health ilk saniyelerde 502, sonra 200.
