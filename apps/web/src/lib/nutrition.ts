...
export function proteinTarget(avgWeightKg: number | null, goals: Goals): ProteinTarget | null {
  ...
  const cutting = goals.weekly_weight_loss_kg > 0
...
/**
 * Bugunun ogunlerini slotlara dagitip acik kalan slotlari dondurur. Atistirmalik
 * gunluk toplama girer ama kendi hedefi yoktur - saat sinirlari cok genis.
 */
export function slotGaps(
  ...
  const gaps: SlotGap[] = []
  for (const slot of MAIN_SLOTS) {
    const consumed_g = todayMeals
      .filter((m) => mealSlot(m.time) === slot)
...
export function weightTrend(
  ...
  const low = weeklyGoalKg > 0 ? weeklyGoalKg * 0.5 : -0.3
  const high = weeklyGoalKg > 0 ? weeklyGoalKg * 1.5 : 0.3
  const status = actual_kg > high ? 'too_fast' : actual_kg < low ? 'too_slow' : 'on_track'
  return {
    kind: 'weight_trend',
    actual_kg,
    target_kg: weeklyGoalKg,
    delta_kg: Number((actual_kg - weeklyGoalKg).toFixed(2)),
    status,
